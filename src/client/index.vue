...
const nodes0 = ref({});
const nodes1 = ref({});
const link0 = ref({source: {}, target:{}});
import { toRaw } from 'vue';

setTimeout(()=>{
  nodes0.value = JSON.parse(JSON.stringify(toRaw(nodes[0])));
  nodes1.value = JSON.parse(JSON.stringify(toRaw(nodes[1])));

  link0.value = {source: toRaw(nodes0.value), target: toRaw(nodes1.value)};
}, 100);
